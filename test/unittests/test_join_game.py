...
class TestJoinGame():
    ...
    @staticmethod
    def test_join_game_should_return_403_if_password_doesnt_match(dao_mock, game):
        ...
        dao_mock.get.assert_called_once_with(id_=id_)
        assert res == (403, "Passwords don't match", {})
        dao_mock.update.assert_not_called()
    ...
    def test_join_game_with_password_should_include_player(dao_mock, game):
        ...
        assert res == (200, "Joined Game",
                       {"Game": {'id_': 'ee81aa59cf1b41ea979e51f5170a128b',
                                 'creation_datetime': dict(game).get("creation_datetime"),
                                 'last_modified_datetime': dict(game).get("last_modified_datetime"),
                                 'pontuacao': [0, 0],
                                 'senha': '123',
                                 'times': [[], []], 'partidas': [],
    ...
    def test_join_game_with_empty_password_should_work_with_any_password(dao_mock,
            game, senha):
        game.senha=""
        dao_mock.get.return_value = game
        res = game_api.join.__wrapped__(id_=id_,
    ...
    @staticmethod
    def test_join_game_should_return_412_if_user_is_already_in_game(dao_mock, game):
        ...
        assert res == (412, "User is already in game", {"Game": dict(game)})
        dao_mock.update.assert_not_called()
